...
var app = {
    ...
    onDeviceReady: function() {
       ...
       // console.log('calling setup push');
        // app.setupPush();
        // Calling the onesignal Initialization function
        app.oneSignalInit();
		//app.onBackKeyDown();
        //Check Login
...
//		var options = new ContactFindOptions();
    ...
    oneSignalInit:function(){
        ...
        window.plugins.OneSignal
            .startInit("1146d78d-655a-4901-a1fb-f65f626b624b")
            .inFocusDisplaying(window.plugins.OneSignal.OSInFocusDisplayOption.Notification)
            .iOSSettings(iosSettings)
            .handleNotificationOpened(notificationOpenedCallback)
            .endInit();

        //To store the user token in local storagess
        window.plugins.OneSignal.getIds(function(ids) {
            // alert("player id: " + ids.userId);
            setStorage('registrationId', ids.userId);
        });
	...
	$(document).on( "click", ".loginBtn", function() {
		...
		request.done(function( data ) {
			...
			if(obj.msg === 'success') {
				...
				$('#logoutFixed').show();
                getAlumnsPage(obj.data.user_id);
                //Get the oneSignal user token and Call the SetUserToken API
                var userToken = getStorage('registrationId');
                setUserToken(obj.data.user_id,userToken);
			}
			else {
				//show error
				//alert(obj.data);
